...
import { Sparkles, Video, PlayCircle, Music } from 'lucide-react';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const videoData = {
        ...
        songId: lastSong.id,
        prompt: fullPrompt,
        videoFileUrl: result.videoDataUri,
        createdAt: new Date().toISOString(),
        style: selectedStyle
      };

      await setDoc(doc(db, 'users', user.uid, 'aiGeneratedSongs', lastSong.id, 'aiGeneratedVideos', videoId), videoData);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-32">
      ...
      {availableSongs.length > 0 ? (
        <div className="p-4 rounded-xl bg-accent/10 border border-accent/20 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="truncate text-sm font-medium">{availableSongs[0].title}</p>
          </div>
        </div>
      ) : (
      ...
      <Button 
        ...
        disabled={isGenerating || availableSongs.length === 0}
